...
Index *sqlite4CreateIndex(
  ...
  */
  ...
  printf("newIndex: created index %s on table %s, idxIsVector %d\n", pIndex->zName, pTab->zName, pIndex->idxIsVector);
  ...
  if( isVectorIndex ){
    ...
    if( pParse->nested || db->init.busy ){
      ...
      skipRefill = 1;
      pIndex->idxIsVector = 1;
      printf("vector index: schema/nested - mark only (no create)\n");
    }else{
